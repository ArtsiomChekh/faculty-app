...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query("SELECT s FROM Student s JOIN DepartmentStudent ds ON s.id = ds.student.id JOIN StudentSubject ss " +
            "ON s.id = ss.student.id WHERE ds.department.id = :departmentId AND ss.subject.id = :subjectId")
